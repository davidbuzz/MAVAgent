...
var box = blessed.box({
  ...
  width: '100%',
  height: '20%',
  content: 'Hello from {bold}Mav-Agent{/bold}!',
  tags: true,
  border: {
    type: 'line'
  },
...
console.log = function(t1,t2,t3){

    //if (t1 1== undefined ) return ; 

    var l =  box2content.length;

    if (l > 20) box2content.shift();// drop oldst element from front

    if (t1 !== undefined ) box2content.push(t1+l+"\n"); // push new content to the end
    if (t2 !== undefined ) box2content.push(t2+l+"\n"); // push new content to the end
    if (t3 !== undefined ) box2content.push(t3+l+"\n"); // push new content to the end

    const reducer = (accumulator, currentValue) => accumulator + currentValue; // appending strings from arry into 1

    if (l < 20 ) box2.setContent(box2content.reduce(reducer) );
           ...
           let dot = dash[2].split(".");
            if(dot.length == 2) {
              ...
              if( typeof modules[key] === 'function') {
                   ...
                   // var libInstance = new modules[key](mavlinkParser2);

                    //console.log("BETTER",libInstance);
                }
...
process.stdin.on('readable', function () {
  ...
  var strcmd = String.fromCharCode.apply(null, partialine); // turn array of bytes into a string
    
  box3.setContent("\nMAVJS-"+MODE+">"+SYSID+"> "+strcmd); 


  // newline pressed on its own means 
  if (data[0] == 0x0d) {
...
var generic_message_handler = function(message) {
    ...
    if ( ! ['GPS_RAW_INT', 'VFR_HUD', 'ATTITUDE', 'SYS_STATUS', 'GLOBAL_POSITION_INT', 'HEARTBEAT','VIBRATION',
            ...
            'AIRSPEED_AUTOCAL', 'MISSION_ITEM_REACHED' , 'STAT_FLTTIME' ,'AUTOPILOT_VERSION' ,
             'FENCE_STATUS' , 'AOA_SSA' , 'GPS_GLOBAL_ORIGIN', 'TERRAIN_REQUEST', ].includes(message.name) ) { 
