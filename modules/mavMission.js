...
function missionRequestHandler(missionItemRequest) {
    //console.log(missionItems.len());
    console.log(missionItemRequest.seq);

    if ( missionItems[missionItemRequest.seq] ) { 
	    mavlinkParser.send(missionItems[missionItemRequest.seq]);
    } else { 
...
// Handler when the ArduPilot requests individual waypoints: upon receiving a request,
// Send the next one.
function missionCountHandler(missionCount) {

    console.log(`MISSION_COUNT number of mission items:= ${missionCount.count} `);

    // just resize array up if needed
    missionItems[missionCount.count-1] = 'last';

     console.log('loadMission-start');
     console.log(missionItems[missionCount.count-1]);
}
// Handler when the ArduPilot requests individual waypoints: upon receiving a request,
// Send the next one.
function missionItemHandler(missionItem) {

    console.log(`updating missionItem ${missionItem.seq} `);


    // AFTER WE GET LAST MISSION ITEM, load PACK TO vehicle as a test

    if ( missionItems[missionItem.seq] == 'last' ) { 
        console.log('loadMission-end');
        console.log(missionItems[missionItem.seq]);
        loadMission();
    }

    missionItems[missionItem.seq] =  missionItem;
...
//MISSION_ITEM_INT seq= 0 command= 16 x= -273917174 y= 1524642186 z= 104.0999984741211
	...
	// attach mission_request handler, let it cook
	mavlinkParser.on('MISSION_COUNT', missionCountHandler);

	// attach mission_request handler, let it cook
	mavlinkParser.on('MISSION_ITEM_INT', missionItemHandler);
